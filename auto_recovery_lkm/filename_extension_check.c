...
char *check_fe(char *file_path, char *copy_path)
{
    ...
    char *str_tok = strsep(&copy_path, ".");
    char *final_tok = NULL;
    int iter = 0, flag = 0;

    // printk(KERN_ALERT "  [*] checking... original path : %s\n", file_path);
    // printk(KERN_ALERT "  [*] checking... copy path : %s\n", copy_path);
    while (str_tok != NULL)
    {
        final_tok = str_tok;
        // printk(KERN_ALERT "  [*] check tok : %s\n", final_tok);
        str_tok = strsep(&copy_path, ".");
    }
    // printk(KERN_ALERT "  [*] final tok : %s\n", final_tok);

    for (iter = 0; iter < (int)ARRAY_SIZE(target_ext); iter++)
    {
        // printk(KERN_ALERT "  [*] compare %s , %s\n", final_tok, target_ext[iter]);
